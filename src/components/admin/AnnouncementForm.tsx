...
export function AnnouncementForm({ onSuccess, onCancel }: AnnouncementFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.title_uz || !formData.title_ru || !formData.title_en ||
        !formData.content_uz || !formData.content_ru || !formData.content_en || !formData.ends_at) {
    ...
    try {
      ...
      selectedFiles.forEach(file => fd.append('medias', file));
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl font-display">
            {language === 'uz' ? 'E\'lon yaratish' : language === 'ru' ? 'Создать объявление' : 'Create Announcement'}
          </CardTitle>
        </CardHeader>
        <CardContent>
